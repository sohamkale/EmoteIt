...
export default function EmortionView({emortion, answeringInterface, GetEmortion}) {
    ...
    function LikeEmortion(){
        setReacting(true);
        axios.put(`/api/emortion/emortion/${emortion?._id}`,null,{headers:
                                ...
                                <button type="button" className="btn btn-light" onClick={LikeEmortion}>
                                    {
                                        ...
                                        emortion?.reactionIds?.some(x=>x._id == user?._id)?
                        ...
                        <div className="col-4 border-right">
                            <div className="btn-group dropright">
                                <button type="button" className="btn btn-light dropdown-toggle"
                                    ...
                                    <span className="small">+{emortion?.reactionIds?.length} Reacts</span>
                                </button>
                                <div className="dropdown-menu">
                                    {/*Reactions Here*/}
                                    {
                                        emortion?.reactionIds?.map((item, index)=>
